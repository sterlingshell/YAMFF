...
fun moveTask(taskId: Int, displayId: Int) {
    ...
    }.onFailure { log("Scheduler", "moveTask failed", it) }
}
...
fun startActivity(context: Context, componentName: ComponentName, userId: Int, displayId: Int) {
    ...
    }.onFailure { log("Scheduler", "startActivity failed", it) }
}
    ...
    when (ConfigManager.config.windowfy) {
        0 -> {
            runCatching {
                ...
                Toast.show("can't move task $taskId")
            }
        }
        1 -> {
            runCatching {
                ...
                Toast.show("can't start activity $componentName")
            }
        }
        2 -> {
            runCatching {
...
fun LaunchRequest.launch(displayId: Int) {
    when {
