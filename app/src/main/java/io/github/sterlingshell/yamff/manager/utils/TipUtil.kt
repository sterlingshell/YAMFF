...
object TipUtil {
    ...
    fun showSnackbar(coordinatorLayout: CoordinatorLayout, @StringRes resId: Int) =
        showSnackbar(coordinatorLayout, AppHolder.app.getText(resId))

    fun showSnackbar(coordinatorLayout: CoordinatorLayout, t: Throwable) {
        runMain {
            val msg = t.localizedMessage ?: t.message ?: AppHolder.app.getString(R.string.unknown_error)
    ...
    fun showTip(context: Context?, @StringRes resId: Int) =
        showTip(context, AppHolder.app.getText(resId))
}
