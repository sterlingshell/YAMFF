...
class GestureHandler(
    ...
    private fun clampRect(rect: Rect, state: State) {
        ...
        if (state.isCollapsed) {
            ...
            val scale = state.scaleFactor * (if (state.isMini) Controller.MINI_SCALE else 1f)
    ...
    fun handleFling(velocityX: Float, velocityY: Float) {
        ...
        runCatching {
            ...
            val flingX = FlingAnimation(FloatValueHolder(state.windowRect.left.toFloat())).apply {
                ...
                addEndListener { _, _, value, _ ->
                    ...
                    if (value <= -windowWidth || value >= screenWidth) {
                        onAction(Action.Minimize)
                    } else {
                        updateGhostIcon(currentState.windowRect)
                        if (currentState.ghostIconVisible) onAction(Action.Minimize)
                    }
                }
            }
            ...
            Log.e("GestureHandler", "handleFling failed", e)
        }
    }
    ...
    fun snapToEdge(isDestroyedProvider: () -> Boolean) {
        ...
        val screenWidth = context.resources.displayMetrics.widthPixels
        val bubbleWidth = if (state.bubbleWidth > 0) state.bubbleWidth else Controller.BUBBLE_WIDTH_COLLAPSED_DP.dpToPx().toInt()
