...
import io.github.sterlingshell.yamff.manager.ui.settings.SettingsScreen
import io.github.sterlingshell.yamff.manager.ui.settings.SettingsViewModel

enum class YAMFFDestination(
    val route: String,
...
@Composable
fun YAMFFAppRoot(settingsViewModel: SettingsViewModel = viewModel()) {
    val navController = rememberNavController()
    var currentDestination by remember { mutableStateOf(YAMFFDestination.Home) }
    ...
    CompositionLocalProvider(
        ...
        NavigationSuiteScaffold(
            navigationSuiteItems = {
                YAMFFDestination.entries.forEach { destination ->
                    item(
                        icon = {
                            Icon(
            ...
            NavHost(
                ...
                startDestination = YAMFFDestination.Home.route
            ) {
                composable(YAMFFDestination.Home.route) { HomeScreen() }
                composable(YAMFFDestination.Settings.route) { SettingsScreen() }
                composable(YAMFFDestination.About.route) { AboutScreen() }
            }
        }
    }
}
