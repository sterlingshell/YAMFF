...
import io.github.sterlingshell.yamff.manager.base.BaseSimpleAdapter
...
import io.github.sterlingshell.yamff.common.extensions.resetAdapter
import io.github.sterlingshell.yamff.common.extensions.runIO
import io.github.sterlingshell.yamff.common.extensions.runMain
import io.github.sterlingshell.yamff.xposed.utils.AppInfoCache
...
import io.github.sterlingshell.yamff.xposed.utils.extensions.startActivity

@SuppressLint("ClickableViewAccessibility")
class AppListWindow(val context: Context, val displayId: Int? = null) {
    companion object {
        const val TAG = "YAMFF_AppListWindow"
        
        // BUG Fix: Keep reference to the window to prevent GC.
        private var instance: AppListWindow? = null
        
        fun show(context: Context, displayId: Int? = null) {
            instance?.closeImmediate()
            instance = AppListWindow(context, displayId)
        }
    }
    ...
    inner class Adapter : BaseSimpleAdapter<ItemAppBinding>(context, ItemAppBinding::class.java) {
        override fun initViews(baseBinding: ItemAppBinding, position: Int) {
            ...
            baseBinding.ll.setOnClickListener {
                ...
                if (displayId == null)
                    YAMFFServer.createWindow(StartCommand(activityInfo.componentName, userId, startRect = rect))
                else
